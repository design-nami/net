...
document.addEventListener('DOMContentLoaded', function () {
  // PC だけでホバー連動させる（タッチデバイスはスキップ）
  if (window.matchMedia && window.matchMedia('(pointer:coarse)').matches) {
    return;
  }

  // data-project ごとに hover 対象をまとめる
  var hoverMap = {};
  ...
  hoverItems.forEach(function (el) {
    ...
    if (!id) return;
    if (!hoverMap[id]) hoverMap[id] = [];
    hoverMap[id].push(el);
  });

  function collectTargets(baseId) {
    var ids = [baseId];
    var bookId = 'book-' + baseId;
    if (hoverMap[bookId]) ids.push(bookId);
    if (baseId.indexOf('book-') === 0) {
      var plain = baseId.slice(5);
      if (hoverMap[plain]) ids.push(plain);
    }
    var all = [];
    ids.forEach(function (id) {
      var arr = hoverMap[id];
      if (arr) {
        arr.forEach(function (el) { all.push(el); });
      }
    });
    return all;
  }
  ...
  // A-3 グラス → M-2 の →やすらぐグラスや←
  var a3 = document.querySelector('.a-3-wrapper');
  if (a3) {
    var glassTargets = collectTargets('palindrome-glass');
